...
from modun.random_restart_hill_climbing import RANDOM_RESTART_HC
from modun.local_beam_search import LOCAL_BEAM_SEARCH
from modun.simulated_annealing import SIMULATED_ANNEALING
from modun.part_of_bfs import BFS_1_PHAN  
from modun.and_or_graph_search import And_Or_Search
...
ALG_MAP = {
    ...
    "RANDOM_HC": RANDOM_HC(),
    "RANDOM_RESTART_HC": RANDOM_RESTART_HC(),
    "LOCAL_BEAM_SEARCH": LOCAL_BEAM_SEARCH(),
    "SIMULATED_ANNEALING": SIMULATED_ANNEALING(),
    "BFS_1_PHAN": BFS_1_PHAN(),
...
class AlgoScrollPanel:
    """Thanh cuộn chọn thuật toán."""

    CATS = {
        "uninformed": ("TÌM KIẾM KHÔNG CÓ THÔNG TIN", (20, 35, 70),  (60, 130, 220)),
        "informed":   ("TÌM KIẾM CÓ THÔNG TIN",        (20, 50, 30),  (50, 200, 110)),
        "local":      ("LOCAL SEARCH",              (50, 35, 12),  (220, 140, 50)),
        "other":      ("KHÁC",                      (40, 40, 40),  (120, 120, 120)),
    }
    ALGO_CAT = {
        "BFS": "uninformed",          "BFS Fast": "uninformed",         
        "DFS": "uninformed",          "DFS Optimal": "uninformed",
        "UCS": "uninformed",          "IDS": "uninformed",
        "IDS EARLY": "uninformed",
        "GREEDY ALGORITHM": "informed","A STAR": "informed","IDA STAR": "informed",
        "SIMPLE_HC": "local",         "IGNORANT_HC": "local",
        "RANDOM_HC": "local",         "RANDOM_RESTART_HC": "local",
        "LOCAL_BEAM_SEARCH": "local", "SIMULATED_ANNEALING": "local",
        "BFS_1_PHAN": "other",        
        "And_Or_Search": "other",
    }

    def __init__(self, x, y, w, h, algorithms):
        ...
        # Xây dựng danh sách mục (header + item xen kẽ)
        self._items = []
        last_cat = None
        for name in algorithms:
            cat = self.ALGO_CAT.get(name, "uninformed")
            if cat != last_cat:
                self._items.append(("header", self.CATS[cat][0], cat))
                last_cat = cat
            self._items.append(("algo", name, cat))

        self._content_h = sum(
            self.header_h if it[0] == "header" else self.item_h + self.gap
            for it in self._items
        ) + 8

    def _item_at_rel_y(self, rel_y):
        cy = 4
        for it in self._items:
            h = self.header_h if it[0] == "header" else self.item_h + self.gap
            if cy <= rel_y < cy + h:
                return it
            cy += h
        return None

    def handle_event(self, event, current_algo, running):
        """Trả về tên thuật toán mới nếu click, ngược lại None."""
        mx, my = pygame.mouse.get_pos()
        if event.type == pygame.MOUSEBUTTONDOWN and not running:
            if event.button == 1 and self.rect.collidepoint(mx, my):
                rel_y = my - self.rect.y + self.scroll_y - 4
                clicked = self._item_at_rel_y(rel_y)
                if clicked and clicked[0] == "algo":
                    return clicked[1]
        if event.type == pygame.MOUSEWHEEL and self.rect.collidepoint(mx, my):
            max_scroll = max(0, self._content_h - self.rect.height + 8)
    ...
    def draw(self, surf, current_algo):
        ...
        cy = self.rect.y - self.scroll_y + 4
        ...
        for it in self._items:
            ...
            if typ == "header":
                h = self.header_h
                if self.rect.y <= cy + h and cy <= self.rect.y + self.rect.height:
                    _, bg_col, fg_col = self.CATS[cat]
                    # Dải màu mỏng bên trái header
                    pygame.draw.rect(surf, fg_col,
                                     pygame.Rect(self.rect.x + 6, cy + 6, 2, h - 12),
                                     border_radius=1)
                    lt = FONT_XS.render(label, True, fg_col)
                    surf.blit(lt, (self.rect.x + 12, cy + (h - lt.get_height()) // 2))
                cy += h

            else:  # "algo"
                h = self.item_h
                if self.rect.y <= cy + h and cy <= self.rect.y + self.rect.height:
                    row_rect = pygame.Rect(self.rect.x + 4, cy, self.rect.width - 8, h)

                    is_active  = (label == current_algo)
                    rel_my     = my - self.rect.y + self.scroll_y - 4
                    is_hovered = (self.rect.collidepoint(mx, my) and
                                  self._item_at_rel_y(rel_my) == it)

                    if is_active:
                        pygame.draw.rect(surf, (15, 30, 60), row_rect, border_radius=5)
                        pygame.draw.rect(surf, (58, 110, 181), row_rect, width=1, border_radius=5)
                        # Thanh accent trái
                        pygame.draw.rect(surf, C["accent"],
                                         pygame.Rect(self.rect.x + 5, cy + 5, 3, h - 10),
                                         border_radius=2)
                    elif is_hovered:
                        pygame.draw.rect(surf, (28, 32, 48), row_rect, border_radius=5)
                        pygame.draw.rect(surf, (60, 68, 100), row_rect, width=1, border_radius=5)

                    tc = C["accent"] if is_active else ((200, 210, 230) if is_hovered else C["text_dim"])
                    name_surf = FONT_XS.render(label, True, tc)
                    surf.blit(name_surf, (self.rect.x + 14, cy + (h - name_surf.get_height()) // 2))

                    # Badge dot khi active
                    if is_active:
                        dot = FONT_XS.render("●", True, C["accent"])
                        surf.blit(dot, (self.rect.x + self.rect.width - dot.get_width() - 8,
                                        cy + (h - dot.get_height()) // 2))

                cy += h + self.gap

        # Scrollbar
        if self._content_h > self.rect.height:
            sb_h    = max(28, int(self.rect.height ** 2 / self._content_h))
            max_scr = max(1, self._content_h - self.rect.height + 8)
            sb_y    = int(self.scroll_y / max_scr * (self.rect.height - sb_h))
            sb_rect = pygame.Rect(self.rect.right - 5, self.rect.y + sb_y, 3, sb_h)
            pygame.draw.rect(surf, (60, 68, 110), sb_rect, border_radius=2)
...
class Visualizer:
    def __init__(self):
        ...
        self.GRID_MAX   = 300
        self.RIGHT_X    = 390
        self.RIGHT_W    = WIDTH - self.RIGHT_X - 100

        # ── Thanh cuộn thuật toán ──────────────────────────────────────
        self.algo_panel = AlgoScrollPanel(
            x=self.PANEL_LEFT,
            y=105,
            w=360,
            h=145,
    ...
    def draw(self, surf):
        ...
        hint = "Cuộn chuột để xem thuật toán  |  Click ô để vẽ/xóa vật cản  |  Nhấn RANDOM để tạo lưới mới"
...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            # Thanh cuộn thuật toán xử lý event trước
            new_algo = visualizer.algo_panel.handle_event(
                event, visualizer.current_algo, visualizer.running_simulation
            )
            if new_algo:
                visualizer.current_algo = new_algo
                visualizer.reset_env()
                continue
